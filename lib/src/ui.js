...
import { killPreviousSound, playSound } from "./audio.js";
import { loadConfig, saveConfig } from "./config.js";
import { DEFAULT_PACK_NAMES } from "./constants.js";
import { downloadPack, fetchRegistry, getPacksDir, listPacks, loadManifest } from "./packs.js";
/** Download packs; `onProgress` receives one-line updates. */
export async function runInstall(packNames, onProgress, isCancelled = () => false) {
...
export function previewPackSound(packName) {
...
export function getPackSoundFile(packName, category = "session.start") {
    const packsDir = getPacksDir();
    const packPath = join(packsDir, packName);
    const manifest = loadManifest(packPath);
    if (!manifest)
