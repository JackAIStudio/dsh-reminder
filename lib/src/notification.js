...
import { detectPlatform, detectPwshBin } from "./platform.js";
...
export function escapeNotificationText(text) {
    return text.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
}
export function detectNotifier(platform = detectPlatform(), commandExists = defaultCommandExists) {
    switch (platform) {
        case "mac":
            return "osascript";
        case "linux":
...
export function resolveIcon(packPath) {
    if (packPath) {
        const packIcon = join(packPath, "icon.png");
        if (existsSync(packIcon))
            return packIcon;
    }
    return DEFAULT_ICON_PATH;
}
export function buildNotifyCommand(notifier, title, body, iconPath, status, promptLine) {
...
function buildWinFormsCommand(title, body, iconPath, status, promptLine) {
