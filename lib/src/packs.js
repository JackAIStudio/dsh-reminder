...
import { LEGACY_PACKS, PACKS_DIR, FALLBACK_REPO, FALLBACK_REF, REGISTRY_URL } from "./constants.js";
export function getPacksDir() {
    if (existsSync(PACKS_DIR) && readdirSync(PACKS_DIR).length > 0)
        return PACKS_DIR;
    if (existsSync(LEGACY_PACKS) && readdirSync(LEGACY_PACKS).length > 0)
        return LEGACY_PACKS;
    return PACKS_DIR;
}
...
export function pickSound(category, config, state) {
...
export async function downloadPack(packName, registry, onProgress) {
    ...
    for (let i = 0; i < filenames.length; i += 5) {
        ...
        const results = await Promise.all(batch.map((f) => downloadFile(`${baseUrl}/sounds/${f}`, join(soundsDir, f))));
