...
export function getRelayUrl(relayMode) {
    if (relayMode === "local")
        return null;
...
export async function relayPlayCategory(relayUrl, category) {
    try {
        const resp = await fetch(`${relayUrl}/play?category=${encodeURIComponent(category)}`, { signal: AbortSignal.timeout(5000) });
        return resp.ok;
    }
    catch {
        return false;
...
export async function relayNotify(relayUrl, title, body) {
    try {
...
export function relaySetupInstructions(session) {
