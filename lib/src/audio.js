...
import { detectLinuxPlayer, detectPlatform, detectPwshBin, detectWindowsPlayer, wslToWindowsPath } from "./platform.js";
import { saveState } from "./config.js";
import { resolveIcon, sendDesktopNotification } from "./notification.js";
import { getPacksDir, pickSound } from "./packs.js";
import { getRelayUrl, relayPlayCategory, relayNotify } from "./relay.js";
const PLATFORM = detectPlatform();
let currentSoundPid = null;
export function killPreviousSound() {
...
let wslNativeChecked = null;
function wslNativeAvailable() {
    if (wslNativeChecked !== null)
        return wslNativeChecked;
    wslNativeChecked = false;
    try {
        if (!existsSync("/mnt/wslg/PulseServer"))
            return false;
 ...
 */
function playWslPowerShell(file, volume, waitSeconds) {
 ...
 */
function playWslSound(file, volume, waitSeconds) {
            ...
            if (Date.now() - started < 300) {
                setTimeout(() => {
                    ...
                    if (fallback) {
                        ...
                        fallback.on("exit", () => {
                            if (currentSoundPid === fallback.pid)
                                currentSoundPid = null;
                        });
                    }
                }, 200);
...
export function playSound(file, volume, waitSeconds = 2) {
    killPreviousSound();
    let child;
    switch (PLATFORM) {
        case "mac":
...
'@
    ...
    if (child) {
        ...
        child.on("exit", () => {
            if (currentSoundPid === child.pid)
                currentSoundPid = null;
        });
    }
}
export function sendNotification(title, body, config, uiNotify, status, promptLine) {
...
export function playCategorySound(category, config, state) {
